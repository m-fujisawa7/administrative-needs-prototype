...
募集中・実施中のものだけを載せる一覧は、募集が途切れた期間に候補が0件になります。これは異常ではなく正常状態なので、台帳で `allow_empty_candidates: true` を設定した情報源だけ、`link_selector` に一致するリンクが0件でもErrorにせず0件の結果を返します。0件だった事実はWARNINGとして残るため、statusはOKではなくWarningになりますが終了コードは0です。

緩めるのはこの1ケースだけです。`link_selector` が未設定、CSSとして不正、タイトルと公式ドメイン内URLを持つ候補が0件、`title_includes` / `title_excludes` の適用後に0件は、いずれも設定の誤りや除外語の効きすぎを示すシグナルなので、`allow_empty_candidates` を設定していても従来どおりErrorです。

0件で正常終了した実行には候補が1件も無いため、本文取得、PDF抽出、AI判定、Notion登録へ進まず `--limit` も消費しません。収集状態の更新条件は変えていないので、`--write` かつ `--since` 無しなら進み、Previewでは進みません。
