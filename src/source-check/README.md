...
`single_page` は入口ページ自体が情報本体で、ページ内の項目が個別URLを持ちません。そのため**Sourceの公式URLだけを常に1件の候補として返します**。ページ内のリンクは候補にしません。取得は疎通確認とURLの生存確認のために行い、本文とPDFの抽出は後続のAI判定が担うため `content_selector` は「未確認」のままです。

掲載日は取得せず `null` にします。ページ内の更新日を掲載日として使うと、初回収集開始日より前に更新されたページが期間フィルタで落ち、一度も解析されないまま静かに取りこぼします。掲載日不明の候補はNoticeを出したうえで必ず処理へ回るため、`initial_since` と手動 `--since` は `single_page` では絞り込みに効きません。**`single_page` に `initial_since` を設定しないでください。**

2回目以降に同じページをAIへ送らないための仕組みは持ちません。登録済みURLは公式URLの重複としてHTML取得・PDF抽出・Claude解析・Notion登録の前に除外されるため、既存の重複判定だけで「初回は解析し、以降は実行しない」が成立します。HTTPの `Last-Modified` はCMSの再生成時刻を返し内容更新を反映しないため、更新検知には使いません。

**ページ内に独立した複数の案件・課題が並ぶ固定ページは対象外です。** 1ページを1レコードにまとめると粒度が粗くなり、案件単位の判断ができなくなります。重複判定が公式URL単位である以上、候補を分割しても2件目以降がすべて重複になるため、現在の設計では扱えません。有効化するのは、ページ全体が1つの行政ニーズまたは政策シグナルを指す情報源だけにしてください。
