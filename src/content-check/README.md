...
台帳に`title_selector`がある場合は、指定要素の表示テキストを空白正規化してタイトルにします。`innerHTML`は使用しません。セレクターが不正、一致0件、または表示テキストが空の場合は、汎用タイトルへフォールバックせずErrorにします。Source固有設定の異常を検知し、誤った案件名を後続のAI・Notionへ渡さないためです。

`title_selector`未設定時は従来どおり、空でない `h1`、`title` の順で取得し、どちらもなければErrorです。
