...
3日前が2026-07-01より前になる場合は2026-07-01へ丸めます。候補の公開日・更新日が対象期間内なら処理対象です。日付が取れない候補は除外せず、警告を表示して重複確認へ進めます。

`initial_since`は初回収集の開始位置だけを変える台帳側の任意設定で、過去分が非常に多く初回からすべてをAI解析したくない情報源に使います。状態ができた後の通常巡回では使わず、丸め先も共通の2026-07-01のままです。`--since`との併用では`--since`が優先され、その場合は従来どおり手動バックフィル扱いで状態を進めません。`initial_since`を使った通常の`--write`は手動バックフィルではないため、完全成功すれば状態を進めます。
