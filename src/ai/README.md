...
4. PDF失敗はWarningにしてHTML本文だけで続行。本文を渡せないPDF（テキストレイヤー無し・パスワード保護）は枠を消費せず次候補を試す
...
本文を1文字も渡せないPDFはAI入力枠を消費させず、次順位の候補を試します。本文を取得できたPDFが最大3件そろった時点で残りの候補は取得しません。

**全ページにテキストレイヤーが無いPDFは、`extractPdfFromBytes` が `no_text` を投げます。** 画像PDFやスキャンPDFがこれに当たり、実運用ではこちらの経路を通ります。取得も解析も成功しており、この文書から得られる本文が0だと確定しているため枠を消費しません。読めなかった事実は `pdf_failed` のWARNINGとして残し、`detail` に `no_text` を入れます（NOTICEへ格下げしません）。OCRは追加していません。

`text.trim() === ''` を見る `pdf_empty_text` のNOTICE経路も残しています。既定の抽出器では上記のとおり `no_text` が先に投げられるためここへは来ませんが、抽出器を差し替えた場合や将来抽出器の契約が変わった場合の防御として残しています。

パスワード保護PDFも同じく枠を消費せず次候補を試します。パスワードの解除・推測・総当たりは行いません。保護されていた事実は `pdf_failed` のWARNINGとして残し、`detail` に `password_protected` を入れます。

**それ以外の取得・解析失敗は、従来どおり枠を消費します。** HTTP取得失敗、`invalid_pdf`、`parse_failed`、`parse_timeout`、`too_many_pages` が該当します。これらは文書を読めたかどうかが確定していないため、試行を枠で制限します。次候補を試しても3件に届かない場合はその件数のまま解析へ進み、渡せるPDFが1件も無くてもHTML本文だけで解析を続けます。

一部のページだけ抽出できない場合は例外にならず、そのPDFは通常どおり採用したうえで `pdf_warning`（`empty_pages`）のWARNINGが付きます。

枠を消費しないケースが多数並ぶページでは候補を多く試行し得ますが、有効な情報源の実測ではPDF候補は1〜7件で実害を確認していないため、試行回数の上限は設けていません。大量試行が実測された場合に独立した改善として検討します。
...
同じPDFを2枠に入れないよう、**URL単位の重複除去とは別に本文の完全一致でも除外します**。URLが違っても中身が同じPDFを公開している情報源があるためです。

正規化は `trim` と連続空白の1文字圧縮だけで、そのSHA-256を fingerprint にします（`pdf-duplicates.ts`）。NFKCも空白の全削除も行いません。判定はChunk選択や切り詰めより前の抽出原文に対して行います。

**完全一致だけを見て、類似度は計算しません。** 導入根拠は福岡市DX戦略の実測で、本編16,881文字と印刷用16,887文字の差は空白だけで、この正規化のみで16,851文字の完全一致になりました。従来はこの2件が3枠のうち2枠を占め、実行項目集が入力へ入りませんでした。

近似判定を入れていないのは、4情報源29ペアを実測して一致したのが上記の1ペアだけで、残りはすべて別文書として区別できていたためです。広島市の第2期DX推進計画・令和6年度改定版・令和6年度取組状況や、富山県の同一タイトル「審査結果について」4件も、いずれも完全一致しません。**近似判定は、正規化しても一致しない真の重複が実データで見つかってから検討します。**

重複PDFは枠を消費せず次候補を試します。残すのは `orderPdfCandidates` の順で先に取得できた方で、ファイル名や「印刷用」といった語による優先判定は持ちません。同一本文である以上どちらを残しても内容が変わらないためです。除外したことは `pdf_duplicate` のNOTICEで示し、文言に採用済みPDFのラベルを含めます。本文は正常に取得できており情報は失われていないため、取得失敗（`pdf_failed`）や本文0文字（`pdf_empty_text`）とは意味が異なります。
...
切り詰めが起きた候補では、AI入力ブロックの `PDF characters` に `20000 (extracted 49846)` の形で抽出全文の文字数を併記します。

## 長大PDFのRelevant Chunk選択

割り当てbudgetを超えるPDFだけ、先頭70%＋末尾30%で機械的に切るのをやめ、行政ニーズ判断に重要そうな原文をbudget以内で選びます。budget以内のPDFは従来どおり全文を渡し、この処理を通しません。AIは使わず決定論的なルールだけで決めるため、同じ入力なら毎回同じ結果になります。

### Chunk単位

PDF抽出のページ本文（`pageTexts`）を使います。`pageTexts.join('\n\n').trim()` が本文と一致する場合だけページ単位で扱い、一致しない場合や1ページがbudgetを超える場合は空行区切りの段落へ落とします。どちらも取れない場合はfallbackします。PDF抽出ライブラリ側は変更していません。
...
| 要素 | 重み |
| --- | --- |
| 重要セクション語を含む見出し | +4（最大4件） |
| 本文に出現する重要セクション語の種類数 | +1（最大5） |
| 低優先セクション語を含む見出し | -3（最大4件） |
| 本文に出現する低優先セクション語の種類数 | -0.5（最大5） |

重要セクション語は目的、背景、趣旨、現状、課題、事業概要、業務概要、業務内容、委託内容、実施内容、要求仕様、機能要件、非機能要件、提案内容、提案事項、企画提案、成果物、期待する効果、官民連携、オンライン手続です。低優先は参加資格、応募資格、提出方法、提出書類、スケジュール、質問方法、契約手続、支払、著作権、個人情報、様式、審査方法、評価基準、採点、問い合わせ先です。低優先セクションも完全除外はせず順位を下げるだけです。

**仕様、要件、成果、実証、構築、開発、運用、保守、改善、BPR、DX、AI、データ、システム、Web、CMS は見出しの中でだけ数えます。** これらは契約条項や事務連絡の本文にも散らばるため、本文出現で加点すると定型条項が高得点になってしまいます。実データでも、この区別を入れる前は契約解除条項が仕様書より高い点数になっていました。

見出しの判定は、番号（`1.`、`（1）`、`第3条`、`一、`、`２ ` など）を外した先頭語が20文字以内で句読点を含まないことを条件にします。番号付きの本文（`(4) 前各号に掲げる場合のほか、…`）を見出しと誤認しないためです。

### 選択と文脈

3段階で選びます。

1. **冒頭の保険**: 先頭から最大1,500文字。案件名や目的が冒頭にあることが多いため少量だけ確保する
2. **得点順**: 1.5点以上のUnitと、その直後のUnit。見出しだけ残って本文が切れる状態を避ける
3. **隣接展開**: 残り予算を既に選んだ範囲の隣へ使う。断片化を減らし前後の文脈を残す

重要箇所が13,000文字で足りる場合、残りを低価値な契約条項や様式で埋めることはしません。選んだUnitは元の順序で連結し、連続していない箇所には切り詰めと同じ省略マーカーを入れます。同じUnitを二重に入れません。

連結後に区切り文字の分でbudgetをわずかに超える場合は、**最も点数の低いUnitを外して収めます。** ここで先頭70%＋末尾30%へ再切り詰めしないのは、選んだ重要箇所が後段で落ちるのを避けるためです。

### fallback

次のいずれかに当たる場合は既存の先頭70%＋末尾30%へ戻します。断片だけを送って情報を失うより従来方式が安全なためです。

- ページも段落もUnitへ分割できない
- 見出しが1つも取れない
- 1.5点以上のUnitが1つも無い
- 連続ブロックが10を超える（断片化しすぎ）
- 選べた量がbudgetの25%を下回る（判定が弱い）

### budgetとの関係

`allocateCharacterLimits` が1PDFあたり20,000文字とPDF合計50,000文字から各PDFのbudgetを決め、そのbudget以内でChunkを選びます。選択後に別処理で切り詰めることはありません。3PDFにそれぞれbudgetが割り当てられる場合も、各budgetに対して個別に選択します。

### ログ

全文を渡したPDFは従来どおり1行です。選択または切り詰めが起きた場合だけ内訳を足します。

```text
PDF input:
- 中部ブロック提案競技募集要項,契約書（案）,仕様書（案）: 19528 chars (extracted 49846, relevant_chunks, 4 chunks)
```

Chunk本文や全文を通常ログへ出すことはありません。

### 実データでの効果

福岡市の水道料金検針・徴収業務のPDF（46ページ、49,846文字。募集要項・契約書案・委託仕様書が1ファイルに結合されている）で確認した。

- 従来の70%＋30%: 募集要項と契約書案の冒頭、末尾の数ページ。契約保証金や暴力団関与の条項が入り、**検針業務委託仕様が落ちていた**
- Relevant Chunk: 19,528文字・4ブロック。募集要項の冒頭、審査・選定、委託仕様書（業務内容・処理要件・体制）、検針業務委託仕様と過誤納金処理を選び、**契約書案16ページと個人情報特記事項・様式類を除外した**

入力文字数の削減は9候補合計で119,174文字から118,702文字（472文字）にとどまる。長大PDFが1件しかないためで、この段階の価値は文字数ではなく中身の選択にある。
...
`evidence_quotes` は `source_url` で指定された原文にだけ当てます。HTMLの引用をPDFへ探しに行くことはありません。照合は正規化後の部分一致で、一致しなければ `evidence_not_found` のWarningになります。

正規化（`normalizeForEvidence`）は引用と原文の両方へ同じ処理を当てます。

1. NFKC
2. 引用符の正規化（曲がり引用符と直線引用符を同一視）
3. 連続する空白を1つへ畳む
4. **空白の左右が両方ASCII英数字の場合を除き、その空白を除去する**
5. trim

手順4があるため、**PDF抽出で混入する `行 政 課 題` や `福 岡 市` のような日本語文字間空白は照合時に吸収されます。** 原文と引用のどちらに空白が入っていても一致します。一方 `open data` と `opendata` のようなASCII英数字間の空白は意味を持ち得るので保持し、区別したまま照合します。

吸収するのは表記の差だけです。**意味的な言い換えは一致させません。** 埋め込み・類似度・あいまい一致は使わず、正規化後の文字列がそのまま含まれるかだけを見ます。
