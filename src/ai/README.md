...
パスワード保護PDFも同じく枠を消費せず次候補を試します。本文を1文字も渡せないため0文字PDFと同じ扱いにしています。パスワードの解除・推測・総当たりは行いません。保護されていた事実は `pdf_failed` のWARNINGとして残し、`detail` に `password_protected` を入れて原因が分かるようにしています（NOTICEへ格下げしません）。

**それ以外の取得・解析失敗は、従来どおり枠を消費します。** 補充は本文0文字とパスワード保護の2つだけです。次候補を試しても3件に届かない場合はその件数のまま解析へ進みます。全PDFが0文字またはパスワード保護でも、HTML本文だけで解析を続けます。OCRは追加していません。

実測では、新潟県のLANシステム用サーバ入札で高優先の別記仕様書がパスワード保護されており、従来は1枠を消費して入札説明書が入力へ入らなかった。補充を入れた後はAI入力PDFが2件・4,414文字から3件・9,673文字になり、入札説明書5,259文字が渡るようになった。
