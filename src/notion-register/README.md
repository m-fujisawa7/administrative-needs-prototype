...
1. 引数、`.env`、情報源台帳、自社適合度基準、プロンプトを読み込む
2. 既存の個別ページ・PDF抽出処理で公開文書を取得する
3. 既存のClaude CLI AnalyzerとZodスキーマで1件を解析する
4. Notionデータベースと全データソースを取得する
5. データソースが1件であることと必須16プロパティの名称・種類を確認する
6. AI内部値をNotion表示値へ変換する
7. `公式URL`のURLフィルターで重複を確認する
8. 予定内容をプレビューする
9. `--write`時だけ`data_source_id`を親として1ページ作成する
...
重複確認は`notion.dataSources.query`を使用し、`公式URL`が完全一致するページを1件検索します。既存ページがあれば正常な登録スキップとして終了し、更新しません。

作成時に送るのは15プロパティです。`登録日時`は`created_time`のため送信せず、Notionの自動値を使用します。ページ本文ブロックや添付ファイルも作成しません。

重複検索とページ作成は別API呼び出しのため、複数プロセスを同時実行した場合の競合を完全には防げません。手動で1件ずつ実行してください。
